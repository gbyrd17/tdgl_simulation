...
class Simulator {
public:
  Simulator(device &device, layer &layer, int resX, int resY,
            int stepsPerFrame = 30);
  virtual ~Simulator();
  ...
  int m_dtUpdateInterval = 100;
  int m_dtUpdateCounter = 0;
  int m_partitionSize = 512;
  int m_vortexCountInterval = 1000;
  int m_vortexCountCounter = 0;
  bool m_enableVortexCounting =
      true; // can be disabled w/ gui to inc. performance
  bool m_renderBothViewports =
      true; // can be disabled w/ gui to inc. performance
...
private:
  ...
  GLuint m_neighborTexture;   // stores neighbor connectivity: (up, down, left,
                              // right)
  GLuint m_edgeWeightTexture; // stores normalized edge distances: (up, down,
                              // left, right)
  int m_readIdx = 0;
  int m_writeIdx = 1;
