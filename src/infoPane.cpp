...
infoPane::infoPane(device &device, Simulator &simulator, layer &layer)
  ...
  gridUnitX =
      (m_device.worldSize.x / (float)m_simulator.m_mesh->resX) / m_layer.xi;
  gridUnitY =
      (m_device.worldSize.y / (float)m_simulator.m_mesh->resY) / m_layer.xi;
}
...
void infoPane::drawLegend() {
  ...
  for (int i = 0; i < 36; i++) {
    float a1 = (i / 36.f) * 2.f * Constants::PI;
    float a2 = ((i + 1) / 36.f) * 2.f * Constants::PI;
    float r, g, b;
...
void infoPane::drawSimVars() {
  ...
  ImGui::Text("Box size    %.2f x %.2f m", m_device.worldSize.x / m_layer.xi,
              m_device.worldSize.y / m_layer.xi);
  ...
  ImGui::Text("N expected  %.1e",
              (m_simulator.mc * m_device.externalB.z * m_device.worldSize.x *
               m_device.worldSize.x) /
                  (2.f * Constants::PI));
  ImGui::Separator();
  ...
  if (normKappa < 1.f)
    ImGui::TextColored(ImVec4(0.4f, 0.8f, 1.f, 1.f), "Type-I");
  else if (normKappa > 1.f)
    ImGui::TextColored(ImVec4(1.f, 0.6f, 0.2f, 1.f), "Type-II");
  else
    ImGui::TextColored(ImVec4(1.f, 1.f, 1.f, 1.f), "Bogomolny point");
  ImGui::Separator();
  ...
  if (ImGui::SliderInt("Partition Size##gpu", &m_simulator.m_partitionSize, 32,
                       1024)) {
    m_simulator.setPartitionSize(m_simulator.m_partitionSize);
  }

  bool noiseCheck = m_simulator.useNoise;
  if (ImGui::Checkbox("Thermal noise", &noiseCheck))
    m_simulator.useNoise = noiseCheck;
  ...
  ImGui::SliderInt("Vortex Count Freq", &m_simulator.m_vortexCountInterval, 100,
                   5000);
  ImGui::SliderInt("Timestep Update Freq", &m_simulator.m_dtUpdateInterval, 10,
                   500);
  ImGui::Separator();

  if (ImGui::Button("Random Quench", ImVec2(-1, 0)))
    m_simulator.quench();
  if (ImGui::Button("Seed Abrikosov Lattice", ImVec2(-1, 0)))
    m_simulator.quenchSeededLattice();
  ImGui::Separator();
  ...
  if (ImGui::Button("Apply Rectangle", ImVec2(-1, 0))) {
    m_simulator.applyGeometryRectangle(geomCenter,
                                       glm::vec2(geomSize, geomSize));
  }
  ...
  float expectedVortices = (m_simulator.mc * m_device.externalB.z *
                            m_device.worldSize.x * m_device.worldSize.x) /
                           (2.f * 3.14159f);
  ImGui::Text("Expected       %.0f", expectedVortices);
  ImGui::Text("Occupancy      %.1f%%", 100.f * m_simulator.vortexCount /
                                           std::max(1.f, expectedVortices));
  ImGui::Separator();
